...
export const atomGetVerticesErrors = atom<IErrorVertices[]>((get) => {
  ...
  vertices.forEach((vertex, index) => {
    ...
    if (isNaN(vertex.forceY)) {
      verticesErrors.push({
        name: 'Fuerza con valor no numerico.',
        message: `El nudo ${vertex.forceY} tiene un valor de fuerza no numerico en el eje Y.`,
        typeError: 'vertices',
        errorCode: 'e110',
        severity: 'error',
      })
    ...
    if (isNaN(vertex.displacementY)) {
      verticesErrors.push({
        name: 'Desplazamiento con valor no numerico.',
        message: `El nudo ${vertex.name} tiene un valor de desplazamiento no numerico en el eje Y.`,
        typeError: 'vertices',
        errorCode: 'e111',
        severity: 'error',
      })
    ...
    if (isNaN(vertex.springY)) {
      verticesErrors.push({
        name: 'Resorte con valor no numerico.',
        message: `El nudo ${vertex.name} tiene un valor de resorte no numerico en el eje Y.`,
        typeError: 'vertices',
        errorCode: 'e112',
        severity: 'error',
      })
    ...
    if (isNaN(vertex.userDOFY)) {
      verticesErrors.push({
        name: 'Grado de libertad con valor no numerico.',
        message: `El nudo ${vertex.name} tiene un valor de grado de libertad no numerico en el eje Y.`,
        typeError: 'vertices',
        errorCode: 'e113',
        severity: 'error',
      })
